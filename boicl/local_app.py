...
<body>
  ...
  <main class="shell">
    <aside class="stack">
      ...
      <section class="panel">
        <h2>Campaign</h2>
        <details id="builtinPresets"><summary>Built-in presets</summary>
          <label for="mocPreset">Preset</label><select id="mocPreset"><option value="moc_five_gp">MoC five-point comparison — six-variable GP · v1.0.0</option><option value="moc_five_llm">MoC five-point comparison — BO-ICL LLM · v1.0.0</option><option value="moc_gp">MoC source-compatible GP continuation</option></select>
          <label for="presetAction">Preset action</label><select id="presetAction"><option value="create">Create new campaign</option><option value="apply">Apply/reset current shared campaign</option><option value="pair">Create matched five-point pair</option></select>
          <p id="presetPreviewSummary" class="hint">Open this panel to validate complete preset settings before creation.</p>
          <details><summary>Effective preset settings and provenance</summary><pre id="presetPreviewConfig" style="white-space:pre-wrap;max-height:320px;overflow:auto"></pre></details>
          <div class="toolbar"><button id="loadMocPreset" disabled>Create campaign from preset</button><button id="loadMocPair">Review matched GP + LLM pair</button></div>
          <p class="hint">The five-point pair has three confirmed seeds and five new measurements per arm, with automatic suggestions off. Creating a preset makes a new campaign. Apply/reset explicitly replaces settings while preserving measurement history. Generic presets use your mapped dataset. No model work starts here.</p>
        </details>
        <details id="savedPresetDetails" class="hidden"><summary id="savedPresetSummary">Saved preset and effective configuration</summary><pre id="savedPresetConfig" style="white-space:pre-wrap;max-height:320px;overflow:auto"></pre></details>
        <div class="field">
      ...
      <section class="panel" id="toolkitSettings">
        ...
        <details id="sharedGPSettings" class="hidden"><summary>Structured GP advanced settings</summary><div class="field"><label for="gpEIThreshold">Start EI after distinct measured designs</label><input id="gpEIThreshold" type="number" min="1"><p class="hint">The five-point MoC preset uses 3, so EI starts with its three seeds. Source-compatible continuation uses 10.</p></div><div class="field"><label for="gpBurnIn">Burn-in steps</label><input id="gpBurnIn" type="number" min="0"></div><div class="field"><label for="gpDraws">Retained draws</label><input id="gpDraws" type="number" min="1"></div><div class="field"><label for="gpThin">Prediction thinning</label><input id="gpThin" type="number" min="1"></div></details>
